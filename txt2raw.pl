...
die "\ntxt2raw.pl V1.0 - Copyright 2005, Vini Matangrano\n"
  ...
  . "Usage: txt2raw.pl <infile> [<outfile>]\n"
...
if ($#ARGV == 1) {
	...
	$outfile =~ s/\.txt$/.raw/ or $outfile .= ".raw";
	
}

open (FILE, $infile) or die "Can't open source file: $infile\n";
open (OUTF,  ">$outfile");
binmode OUTF;
...
while (<FILE>)
  ...
  last if ($error != 0);
  $line++;
  next if (/^#/);									### Comment line - CM
  s/\/\/.*//;										### C++ style comment - CM
